...
class processpool {
 ...
 private:
  /* 将构造函数私有，因此我们只能通过后面的create静态函数来创建processpool实例 */
 ...
 private:
  ...
  /* 子进程在池中的序号,从0开始 */
  int m_idx;
  /* 每个进程都有一个epoll内核事件数，用m_epollfd 标识 */
  int m_epollfd;
  /* 监听socket */
  int m_listenfd;
...
/* 进程池构造函数。参数listenfd是监听socket,它必须在创建进程池之前创建，否则
 * 子进程无法直接饮用它。参数process_number指定进程池中子进程的数量 */
template<typename T>
processpool<T>::processpool(int listenfd, int process_number) : m_listenfd(
    listenfd), m_process_number(process_number), m_idx(-1), m_stop(false) {
...
/* 父进程中m_idx值为-1,子进程中m_idx值大于等于0,我们据此判断接下来要运行的是父进程代码还是子进程
 * 代码 */
template<typename T>
...
void processpool<T>::run_child() {
  ...
  /* 子进程需要箭筒管道文件描述符pipefd, 因为父进程将通过它来通知子进程accept新连接 */
  ...
  while (!m_stop) {
    ...
    for (int i = 0; i < number; ++i) {
      ...
      if ((sockfd == pipefd) && (events[i].events & EPOLLIN)) {
        ...
        int client = 0;
        /* 从父、子进程之间的管道读取数据，并将结果保存到变量client中。
         * 如果读取成功，则表示新客户连接到来 */
  ...
  while (!m_stop) {
    ...
    for (int i = 0; i < number; ++i) {
      ...
      if (sockfd == m_listenfd) {
        /* 如果有新连接到来，就采用Round Robin方式将其分配给一个子进程 */
        ...
        if (ret <= 0) {
          ...
          for (int i = 0; i < ret; ++i) {
            switch (signals[i]) {
              ...
              case SIGTERM:
              case SIGINT: {
                /* 如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部结束。当然，通知
